...
# ---------------------------------------------------------------------------
# Конфиг (сохранение/загрузка настроек между сессиями)
# ---------------------------------------------------------------------------
...
def _load_xml_audio_mapping():
    """Пробует загрузить последнюю XML-конфигурацию ATEM из текущей папки."""
    xml_files = sorted(glob.glob("*.xml"), key=os.path.getmtime, reverse=True)
    if not xml_files:
        return None
    path = xml_files[0]
    try:
...
def _export_xml_with_routing(xml_path: str, state: "RoutingState") -> Optional[str]:
...
class AudioRoutingOutputCommand(Command):
    ...
    def get_command(self):
        ...
        if self.include_ports:
            # Формат AROC (extended): flag(u8), pad(3), output_id(u32), source_id(u32), ext(u16), int(u16), name(64)
            data = struct.pack('>B 3x I I H H 64s', flag, self.output_id, self.source_id, self.ext_port, self.int_port, name_bytes)
        else:
